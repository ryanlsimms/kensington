...
export function githubLink() {
  return t.footer({ class: 'sidebar-footer' }, [
...
export function headerGithubLink() {
  return t.a({
    href: 'https://github.com/ryanlsimms/kensington',
    class: 'header-github',
    target: '_blank',
    rel: 'noopener',
  }, [
...
export function code(lang, src) {
  const escaped = src
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  const btn = renderForHydration(copyButton, {}, 'copyButton').toString();
  // eslint-disable-next-line kensington/no-unsafe-literal -- escaped above
  return t.unsafeLiteral(`<div class="code-wrap"><pre class="language-${lang}"><code class="language-${lang}">${escaped}</code></pre>${btn}</div>`);
}

export function panels(items) {
  return t.div({ class: 'panels' }, items.map(item => {
    const parts = [];
    if (item.label) {
...
export function ideMock(opts) {
  ...
  if (popup) {
    ...
    const errorIcon = t.span({ class: 'ide-error-icon' }, errIconSvg);
    const warnIcon = t.span({ class: 'ide-warn-icon' }, t.svg({
      viewBox: '0 0 16 16',
      width: '13',
      height: '13',
      fill: 'none',
    ...
    const icon = popup.type === 'error' ? errorIcon : warnIcon;
    const typeContent = popup.typeContent ? t.div({ class: 'ide-popup-type' }, t.literal(popup.typeContent)) : null;
    popupNode = t.div({ class: 'ide-popup' }, [
  ...
  let completionNode = null;
  if (completion) {
    completionNode = t.div({ class: 'ide-completion' }, completion.map((item, i) => {
      const itemClass = i === 0 ? 'ide-completion-item ide-completion-selected' : 'ide-completion-item';
      return t.div({ class: itemClass }, [
        t.span(item.name),
  ...
  return t.div({ class: 'ide-mock', ariaHidden: 'true' }, [
    ...
    t.div({ class: 'ide-editor' }, editorLines.map((line, i) => t.div({ class: 'ide-line' }, [
