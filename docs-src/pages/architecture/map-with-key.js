    ...
    t.section({ id: 'map-with-key-render' }, [
      ...
      callout('key', 'Why the keep-alive exists',
        t.p([
          ...
          t.code('keepAwake'),
          ' effect holds a subscription while the list is awake. It pauses when the list sleeps, resumes when the list is read again, and stops permanently when the list stops.',
        ]),
      ),
      t.p([
        'Shared dependency changes coalesce into one synchronous outer list evaluation before the source write returns. ',
        'An immediate read also refreshes a pending list. When all rendered entries are unchanged, ',
        'the previous output array is reused so consumers do not receive a redundant update.',
      ]),
      t.p([
