  ...
  return [
    t.section({ id: 'tooling' }, [
      ...
      t.p([
        ...
        '. It carries no per-element attribute spec data, making it smaller than the full build. Signal-only consumers tree-shake away the class and tag-rendering code; the tree-shaking test enforces a 15 KB minified budget.',
      ]),
...
return new Proxy(this, {
  get(target, prop, receiver) {
    ...
    return fn;
      ...
      t.h3('Reactive core is externalized'),
      t.p([
        'Every module under ',
        loc('esm/lib/reactive/'),
        ' is marked external in the slim rollup. The emitted bundle imports them via relative paths (',
        t.code('../esm/lib/reactive/signal.js'),
        ', ',
        t.code('../esm/lib/reactive/hydration-scope.js'),
        ', etc.) that resolve identically for browsers, Node, and every bundler. Any other consumer of those files — ',
        t.code('kensington/live'),
        ', ',
        t.code('kensington/vite'),
        ', user code that imports ',
        t.code('kensington/reactive'),
        ' — resolves to the same on-disk file, so mutable module state like ',
        t.code('currentEffect'),
        ', ',
        t.code('currentHydrationScope'),
        ', and the SSR-mode counter is shared by exactly one instance. Aliasing ',
        t.code('kensington -> kensington/dist/slim/min'),
        ' alongside ',
        t.code('kensington/live'),
        ' works without effects going silent across the boundary.',
      ]),
      t.p([
