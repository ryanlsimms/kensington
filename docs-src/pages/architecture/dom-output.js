...
let hasSignalContent = false;`),
  ...
  if (val === false || val === null || val === undefined) {
    ...
    el.setAttribute(attrName, String(val));
      ...
      callout('note', 'Reactive class arrays and composed attributes',
        t.p([
          'A class array that mixes static strings with signals (',
          t.code("class: ['card', isActive.transform(a => a ? 'on' : '')]"),
          ') is collapsed by ',
          t.code('deriveClassList'),
          ' into a single derived signal built with ',
          t.code('_internalComputed'),
          ', which joins the resolved entries on each change. The same ',
          t.code('_internalComputed'),
          ' path composes any attribute whose value is an array or object containing signals at depth (',
          t.code('style'),
          ', ',
          t.code('data'),
          ', ',
          t.code('aria'),
          ', and other namespaced slots). The internal form is required so that reads of user-keyed signals inside a row do not trip the out-of-scope or computed-in-computed warnings. The internal run is flagged ',
          t.code('_isInternal'),
          ' and skips both guards.',
        ]),
      ),
    ]),
