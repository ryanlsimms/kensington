...
export function architectureStringOutput() {
  ...
  return !LINE_BREAK_TEST_REGEX.test(content);
      ...
      t.p([
        ...
        t.code('name="value"'),
        ', encoding the value through the ',
        t.code('he'),
        ' encoder wrapper. Booleans render as the bare attribute name (',
        t.code('disabled'),
        ' not ',
        t.code('disabled="true"'),
        '). Function values cannot be serialized to strings and are silently omitted, with the ',
        t.code('handleFunctionValues'),
        ' callback invoking ',
        t.code('showInvalid'),
        ' at that point rather than at tag creation.',
      ]),
      t.h4('Attribute composition'),
      t.p([
        'Before serialization, ',
        t.code('attributesArrayFromObject'),
        ' normalizes the options object into the flat ',
        t.code('[name, value]'),
        ' array that both ',
        t.code('toString()'),
        ' and ',
      ...
      t.ul([
        t.li([
          'camelCase keys convert to kebab-case, and nested namespace objects expand. ',
        ...
        t.li([
          'A ',
          t.code('style'),
          ' object is stringified to a declaration list. camelCase property names convert to kebab-case, and ',
          t.code('null'),
          ', ',
          t.code('undefined'),
          ', and ',
        ...
        t.li([
          'A ',
          t.code('class'),
          ' array is joined to a single string. Falsy entries drop out.',
        ]),
        t.li([
          'The ',
          t.code('prop'),
          ' and ',
          t.code('on'),
          ' keys are skipped here. They never enter the HTML attribute pipeline.',
        ]),
      ]),
    ]),
  ]);
