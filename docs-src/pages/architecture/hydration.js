...
export function computed(fn) {
  ...
  // ... normal path ...
    ...
    t.section({ id: 'hydration-register' }, [
      ...
      t.p([
        'On the client, ',
        t.code('registerComponents({ name: fn }, { context, nonce })'),
        ...
        t.code('context'),
        ' is stored per component name and reused for any later HMR re-render. The optional ',
        t.code('nonce'),
        ' is applied to transient hydration style guards when the page uses a strict Content Security Policy.',
      ]),
      t.p([
        'All registrations share one MutationObserver for the document. Each component name retains its original context and ',
        t.code('nonce'),
        '. Duplicate names produce a warning and are ignored. New names in the same call still register. Registrations last for the document\'s lifetime and return nothing. Hydration groups mounts by nonce before creating style guards. It resolves the function from the shared ',
        t.code('componentRegistry'),
        ' at hydration time, so a dynamically inserted mount uses the latest HMR replacement.',
      ]),
      t.p([
