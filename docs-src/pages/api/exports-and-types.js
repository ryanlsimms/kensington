...
import { signal, computed, effect, applyPendingReactiveUpdates } from 'kensington';
...
import { t } from 'https://cdn.jsdelivr.net/npm/kensington/dist/kensington.min.js';`),
  ...
  // ... one export per element
      ...
      t.p([
        ...
        ', the source ESM — resolves the exact same source reactive modules at runtime when loaded from one package installation, so dependency tracking and pending updates are shared when ESM entry points are mixed. Signal branding alone is not enough because the pending effect queue is module-scoped. Separate package copies and the generated CommonJS runtime remain separate schedulers. Consumers who need only the reactive primitives can import it directly and drop the tag pipeline entirely.',
      ]),
      code('javascript', `import { signal, computed, effect, applyPendingReactiveUpdates } from 'kensington/reactive';`),
