...
export function basicsAdvanced(t) {
  return [
    t.header([
      t.h1('Advanced Usage'),
    ...
    t.section({ id: 'advanced' }, [
      ...
      t.table({ class: 'option-table' }, [
        t.tbody([
          ...
          t.tr([t.td('Nested objects'), t.td([t.code('{ data: { bs: { toggle: \'collapse\' } } }'), ' → ', t.code('data-bs-toggle="collapse"')])]),
          t.tr([t.td('Boolean attributes'), t.td([t.code('{ checked: true }'), ' → ', t.code('checked'), '. ', t.code('{ checked: false }'), ' → attribute omitted.'])]),
          t.tr([t.td('class as array'), t.td([t.code("{ class: ['foo', 'bar'] }"), ' → ', t.code('class="foo bar"')])]),
          t.tr([t.td('data-* and aria-*'), t.td(['Always allowed on every element, along with all ', t.a({ href: 'https://html.spec.whatwg.org/multipage/dom.html#global-attributes' }, 'global HTML attributes'), '.'])]),
          t.tr([t.td('style as object'), t.td([
            t.p([t.code('{ style: { backgroundColor: \'red\', zIndex: 2 } }'), ' → ', t.code('style="background-color: red; z-index: 2"')]),
            t.p(['camelCase keys always convert to kebab-case. CSS property names are always kebab-case (including for SVG); camelCase is only the JavaScript DOM convention for ', t.code('element.style'), '. ', t.code('null'), ', ', t.code('undefined'), ', and ', t.code('false'), ' values are silently omitted. In TypeScript, the style object is typed with ', t.a({ href: 'https://www.npmjs.com/package/csstype' }, 'csstype'), ' for autocomplete on property names and values.']),
          ])]),
          t.tr([t.td('on key'), t.td([t.code('{ on: { myCustomEvent: handler } }'), ' wires listeners via ', t.code('addEventListener'), '. Event names are passed verbatim. Use this for custom or camelCase event names that ', t.code('on*'), ' attributes cannot express. Silently ignored in ', t.code('.toString()'), '.'])]),
          t.tr([t.td('prop key'), t.td([t.code("{ prop: { value: 'hello' } }"), ' assigns directly to DOM properties (', t.code('el.value = ...'), ') instead of ', t.code('setAttribute'), '. Silently ignored in ', t.code('.toString()'), '.'])]),
        ]),
      ]),
...
t.div({ unknownAttr: 'x' });     // throws. Not a known attribute`),
      ...
      code(t, 'javascript', `import Kensington from 'kensington/dist/slim';
...
t.div({ class: 'card' }, t.p('Hello'));`),
      ...
      code(t, 'javascript', `// vite.config.js
...
const t = new Kensington({
  ...
  logger: msg => myLogger(msg),   // receives validation warnings, default console.log
      ...
      t.aside([
        ...
        code(t, 'javascript', `const t = new Kensington({ additionalNamespaces: 'xmlns' });
t.svg({ xmlns: 'http://www.w3.org/2000/svg', 'xmlns:xlink': 'http://www.w3.org/1999/xlink' });`),
      ...
      t.ul([
        t.li([t.strong('Attribute names:'), ' checked against the HTML/SVG/MathML spec. ', t.code('data-*'), ', ', t.code('aria-*'), ', ', t.code('additionalNamespaces'), ', and ', t.code('additionalGlobalAttributes'), ' are always allowed.']),
        t.li([t.strong('Attribute values:'), ' checked against allowed types/literals (e.g. ', t.code('type'), ' on ', t.code('input'), ' only accepts known values; ', t.code('id'), ' must not start with a digit).']),
        t.li([t.strong('Style object values:'), ' non-string/number values (other than ', t.code('undefined'), ') are flagged.']),
      ]),
      code(t, 'javascript', `const t = new Kensington({ validationLevel: 'error' });
...
t.button({ type: 'button', popovertarget: 'my-popover' }, 'Open').toString();`),
      ...
      code(t, 'typescript', `import Kensington, { type ContentMethod } from 'kensington';
...
t.div({ hxBoost: 'true' });  // now valid`),
      ideMock(t, {
        filename: 'index.ts',
        lines: [
          `<span>t</span><span>.</span><span class="ide-t-fn">myCard</span><span>({ </span><span class="ide-t-prop">'card-type'</span><span>: </span><span class="ide-t-str">'primary'</span><span>, </span><span class="ide-t-prop">loading</span><span>: </span><span class="ide-t-str">false</span><span> }, </span><span class="ide-t-str">'content'</span><span>)</span>`,
          `<span>t</span><span>.</span><span class="ide-t-fn">myCard</span><span>({ </span><span class="ide-t-prop">'card-type'</span><span>: </span><span class="ide-squiggly"><span class="ide-t-str">'featured'</span></span><span> }, </span><span class="ide-t-str">'content'</span><span>)</span>`,
        ],
        popup: {
          type: 'error',
          code: 'TS2322',
      ...
      t.p([
        exLink(t, '?page=examples#htmx', 'htmx integration example'),
        ' ',
        exLink(t, '?page=examples#tailwind', 'Tailwind example'),
        ' ',
      ...
      code(t, 'javascript', `// Without persist: true, the reconciler's insertBefore moves during drag-reorder
...
t.inlineComment('line 1\\nline 2');       // <!--\\n  line 1\\n  line 2\\n-->`),
      t.p(exLink(t, '?page=examples#preformatted', 'Preformatted text example')),
    ]),
