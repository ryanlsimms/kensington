...
export function basicsTooling() {
  return t.section({ id: 'tooling' }, [
    ...
    panels([
      {
        label: 'HTML input',
        content: code('html', `<nav class="navbar" aria-label="main" aria-expanded="true">
  ...
  <a href="/about" class="nav-link">About</a>
      ...
      },
      {
        ...
        content: code('javascript', `t.nav({ class: "navbar", aria: { label: "main", expanded: "true" } }, [
  t.a({ href: "/", class: "nav-link" }, "Home"),
  t.a({ href: "/about", class: "nav-link" }, "About"),
])`),
      },
    ]),
    apiTable(['Mode', 'Command'], [
    ...
    t.p([
      ...
      t.a({
        href: 'https://marketplace.visualstudio.com/items?itemName=ryan-lee-simms.kensington',
        target: '_blank',
        rel: 'noopener',
      }, 'VS Code'),
      ' and ',
      t.a({
        href: 'https://plugins.jetbrains.com/plugin/31827-kensington',
        target: '_blank',
        rel: 'noopener',
      }, 'JetBrains'),
      ' IDEs. Both plugins also wire up Go to Definition and Find Usages ',
      'between CSS selectors and Kensington templates.',
    ]),

    t.h3({ id: 'eslint-plugin' }, 'ESLint plugin'),
    t.p([
      t.a({
        href: 'https://www.npmjs.com/package/kensington-eslint-plugin',
        target: '_blank',
        rel: 'noopener',
      }, 'kensington-eslint-plugin'),
      ' catches common signal mistakes at lint time: ',
...
export default [
  ...
  // ...your other configs
    ...
    code('javascript', `import 'kensington/devtools';`),
    devtoolsMock(),
    t.p([
    ...
    t.p([
      'Kensington ships an ',
      t.code('AGENTS.md'),
      ' file at the package root. It is a compact, single-file reference of the full API: ',
      'method signatures, attribute rules, constructor options, TypeScript types, the CLI, and working examples. ',
      'AI coding assistants can read it to answer questions and generate accurate Kensington code.',
    ]),
    t.h4('Using it'),
    t.p([
    ...
    t.ul([
      t.li([
        t.strong('Claude Code'),
        ': reference ',
        t.code('node_modules/kensington/AGENTS.md'),
        " in your conversation, or add it to your project's ",
        t.code('CLAUDE.md'),
        '.',
      ]),
      t.li([
        t.strong('Cursor / Windsurf'),
        ': add the file to your ',
        t.code('.cursorrules'),
        ' context or drag it into the chat.',
      ]),
      t.li([
        t.strong('Any chat interface'),
