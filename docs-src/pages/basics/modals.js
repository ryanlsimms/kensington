import { t, signal, computed, isBrowser } from 'kensington';
...
export function comparisonsModal() {
  ...
  return [
    ...
    t.div({
      class: computed(() => open.get() ? 'modal open' : 'modal'),
      role: 'dialog',
      'aria-modal': 'true',
      'aria-label': 'Why Kensington comparisons',
      onclick: e => {
        ...
        panels(t, [
          {
            ...
            content: code(t, 'javascript', `// TypeScript sees a plain string. No attribute checking inside.
const html = htm\`<input typ="checkbox">\`;
//                       ^^^ typo, no error raised`),
          },
          {
            label: 'Kensington',
            content: code(t, 'typescript', `// t.input knows the InputAttributes interface
t.input({ typ: 'checkbox' });
// TypeScript: 'typ' does not exist on InputAttributes`),
          },
        ]),
        t.h4('vs JSX'),
        t.p('JSX offers similar structure but requires a compiler and a framework pragma. Kensington works anywhere JavaScript runs, such as Node.js, Deno, Bun, and the browser, with no configuration.'),
        t.h4('vs React / Vue / Angular / SolidJS'),
        t.p('Full front-end frameworks are designed around a component lifecycle, a virtual DOM or fine-grained reactivity graph, a router, and a build pipeline.'),
        t.p('Kensington is useful when that scope is more than you need. It generates HTML strings on the server with no runtime dependency, produces live DOM nodes in the browser with no virtual DOM overhead, and works inside Web Components or vanilla JS projects without committing to a framework. The 2.0 signals build adds fine-grained reactivity for the cases where you do need live updates, without pulling in the bathwater with the baby.'),
        t.h4('vs DOM methods'),
        t.p([
        ...
        panels(t, [
          {
            label: 'Hyperscript',
            content: code(t, 'javascript', `// attrs typed as Record<string, any>, no element-specific checking
