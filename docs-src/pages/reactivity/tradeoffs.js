  ...
  return t.section({ id: 'known-tradeoffs' }, [
    ...
    t.ul([
      ...
      t.li([
        ...
        t.strong(' inside a '),
        t.code('computed'),
        t.strong(' replaces the DOM node on every outer re-render.'),
        ' The library swaps in a fresh node so the new signal\'s effect can drive the live element. Focus, scroll, input values, and text selection are preserved across the swap. IME composition, CSS animations in flight, pointer capture, ',
        t.code('<canvas>'),
        ' bitmaps, ',
        t.code('<iframe>'),
        ' documents, web component instance state, and third-party listeners attached outside Kensington are lost. Pass a stable key as the second argument (',
        t.code('signal(initial, key)'),
        ') to scope the signal to the computed so the same instance is reused across re-runs. The DOM node stays in place and nothing is lost.',
      ]),
      t.li([
