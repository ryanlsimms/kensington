...
- The slim bundles (`dist/kensington.slim.js` and `dist/kensington.slim.min.js`) now externalize every module under `esm/lib/reactive/` via relative paths. Consumers can safely alias `kensington -> kensington/dist/slim/min` alongside `kensington/live` (or any other subpath); the reactive core loads exactly once across entry points, so `signal.set()` from one entry point wakes `effect()` registered from another.
...
- `toElement()` now assigns the correct namespace URI to `xlink:*`, `xml:*`, and `xmlns*` attributes, including reactive updates.
- HTML scripts created by `unsafeLiteral().toElement()` now execute or load consistently when inserted. Foreign-content scripts retain native browser behavior.
- SVG attribute maps and types now remain complete, preserve element-specific definitions, and isolate mutable definition arrays between public maps.

## [2.0.0-signals.23] - 2026-07-21
...
### Fixed
- Signal-bound bindings on elements nested inside a conditionally rendered subtree now stay reactive after the subtree is hidden and shown again. Previously the binding could freeze at its last value if the signal lived on a descendant of the conditionally rendered root rather than directly on the root itself. Affects signal-bound attributes, signal-text `inlineComment`s, and `literal()` driven by a signal. The bug was most visible with a sleeping computed in the chain, for example a class driven by `isExpanded` reading both a shared search state and a per-item open flag.
