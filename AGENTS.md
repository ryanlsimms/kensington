...
- **`require-reactive-key`** (error in strict). Flags every unkeyed `signal()`/`computed()`/`.transform()` call site, period. Keys are no-ops at module scope, so passing one is always harmless. The right response is almost always **add a key**, not suppress. A key is one extra string argument; it's refactor-safe (if the call later moves inside a reactive callback, the key starts doing work automatically with no source change); it's a no-op at runtime when the call IS at module scope. If you find yourself writing more than a handful of `// eslint-disable-next-line kensington/require-reactive-key` comments, you're using the wrong tool — just add keys.
