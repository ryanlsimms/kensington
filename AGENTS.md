...
|---|---|---|
| You touch `signal()`, `computed()`, `effect()`, `.transform()`, `mapWithKey`, `addConnectedCallback`, `persist: true`, devtools, or any reactive lifecycle. | [`agent-docs/reactive.md`](agent-docs/reactive.md) | Signal API (read with `.get()`, or `.subscribe()` for trigger-only effect dependencies); keyed lists; the helper-function trap with wrong/right pairs; lazy registries; cleanup; `addConnectedCallback`/`addDisconnectedCallback`; `persist: true` for drag-and-drop; devtools; loading state; **Reactive pitfalls** (every warning's wrong/right pair). |
...
| Wire a "reconnect now" button | `live-signals.md` → Transport lifecycle control |
...
|---|---|---|---|
| `async-loop` | The same effect repeatedly re-enters across queued microtask turns without yielding to a task | Break the cycle. Do not write a signal the effect reads, or guard the write with a value-equality check | `reactive.md` → Reactive pitfalls |
...
| `set-in-effect` | A signal is both read with `.get()` or `.subscribe()` and written with `.set()` in the same effect run | Read once at the top into a local. Compute the new value. Only call `.set()` when it differs | `reactive.md` → Do not read and write the same signal in the same effect or computed run |
...
Everything else — the full Signal API (read with `.get()`, or `.subscribe()` for trigger-only effect dependencies; `.value` is an escape hatch), keyed lists, the helper-function trap with wrong/right pairs, lazy registries, cleanup, `addConnectedCallback`/`addDisconnectedCallback`, devtools, loading state, and the full pitfalls catalogue — is in `agent-docs/reactive.md`.
