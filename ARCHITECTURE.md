...
- **Signal content and direct signal mounting share `reconcile.js`.** Stable keyed tags reuse their nodes. Fresh tags for existing keys replace the old nodes. State is captured for all replacements before DOM mutations; form state is restored before focus and scroll. Scalar text updates reuse text nodes. `stopRemoved` stops removed bindings synchronously, before the MutationObserver fires.
