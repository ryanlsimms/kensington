...
**Stable releases** (from `master`, the 1.x line):
...
**Named prerelease lines** (the `signals` branch is currently the 2.0 line). Pass the preid as a second argument:
...
Prereleases are published to npm under the preid as the dist-tag (`signals`, `beta`, etc.), so `npm install kensington` stays on the latest stable. Users opt in with `npm install kensington@signals`.

The 0.x line is frozen. Tag `v0.15.4` is its final release.
