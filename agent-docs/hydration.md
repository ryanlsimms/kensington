...
Multiple `registerComponents` calls are supported and share one observer. Register each component name once. A duplicate name produces a warning and is ignored, even if the function is the same. Its original function, context, and nonce stay unchanged. New names in the same call still register. Registrations last for the document's lifetime and the function returns nothing. Hydration resolves the latest function from the shared registry, so HMR can update components without registering them again. Each mount uses the nonce from its component's original registration.
...
**DOM replacement, not true hydration.** `registerComponents` replaces the entire SSR DOM with a fresh `toElement()` call rather than reusing existing nodes. In practice the replacement is imperceptible: `replaceWith()` is synchronous and the visual output is identical. After the client component renders and validates successfully, a transient `<style data-k-ssr>` guard suppresses transitions and animations only for the mount IDs being replaced. Kensington keeps the guard through synchronous signal commits, connected callbacks, and application-scheduled mount microtasks, commits all same-frame hydration work in one layout pass, and removes it before paint so later application state changes animate normally. Missing, skipped, and failed mounts retain their SSR DOM without receiving a guard. Dynamically inserted SSR fragments receive their own mount-scoped guard without interrupting motion in components that are already live. If the page's Content Security Policy requires a nonce for inline styles, pass the request's nonce on the client: `registerComponents({ appPage }, { context: env, nonce: cspNonce })`. Kensington applies it to every transient hydration guard created by that registration.
...
The wrap-on-client step is the only place where the server and client diverge. `mapWithKey` also handles the plain-object case (SSE deltas that produce a new ticket object at the same id re-run that row's mapFn via the shallow-diff gate), so the per-row-signal wrap is optional. It's useful when you have per-field `.transform()` chains or want independent update granularity per field; skip it if a whole-row replace per SSE frame is fine.
